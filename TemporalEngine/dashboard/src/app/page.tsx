...
export default function Home() {
  ...
  const { data: metrics_raw }      = useSWR("/api/data/metrics",      fetcher, { refreshInterval: 3000 });
  const { data: alerts_raw }       = useSWR("/api/data/alerts",       fetcher, { refreshInterval: 1500 });

  const rings        = Array.isArray(rings_raw)        ? rings_raw        : [];
  const transactions = Array.isArray(transactions_raw) ? transactions_raw : [];
  const alerts       = Array.isArray(alerts_raw)       ? alerts_raw       : [];
  const metrics      = metrics_raw ?? EMPTY_METRICS;
  const steps        = Array.isArray(steps_raw) ? steps_raw : [
    { id: "idle", node: "Awaiting Trigger", status: "pending" as const,
      message: "Monitoring system for autonomous investigation triggers..." },
  ];

  // Derive live header stats from metrics
  const txPerSec   = metrics.events_processed > 0 ? `${(metrics.events_processed % 1000).toFixed(0)} total` : "—";
  const latencyVal = metrics.active_users > 0      ? `${(metrics.active_users * 14).toFixed(0)}ms`          : "—";
  ...
  return (
    <main className="min-h-dvh background-grid p-8 lg:p-12 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-bold tracking-tight text-balance">
                Temporal Intelligence Hub
              </h1>
            </div>
            <p className="text-sm text-white/40 ml-12">
              Production Command Center v2.0 · sovereign
            </p>
          </div>
          ...
          <div className="flex gap-4">
            <GlassCard className="px-4 py-2 flex items-center gap-3 bg-white/[0.02] border-white/5">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-white/40 uppercase">Events Processed</span>
                <span className="text-sm font-bold tabular-nums">{txPerSec}</span>
              </div>
            </GlassCard>
            <GlassCard className="px-4 py-2 flex items-center gap-3 bg-white/[0.02] border-white/5">
              <Zap className="size-4 text-warning" />
              <div className="flex flex-col">
                <span className="text-[10px] text-white/40 uppercase">Active Users</span>
                <span className="text-sm font-bold tabular-nums">{metrics.active_users}</span>
              </div>
            </GlassCard>
          </div>
        </header>
        ...
        {/* Dashboard Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 xl:grid-cols-16 gap-6 items-start">
        ...
        {/* Alert Feed — full width below the main grid */}
        <AlertFeed alerts={alerts} />

        {/* Footer */}
        <footer className="flex items-center gap-2 text-xs text-white/20 mt-2">
          <Info size={14} />
          <span>
            Real-time data · Flink → Memgraph + Redis · Alerts via{" "}
            <code className="font-mono">fraud:alerts</code> ·{" "}
            {metrics.active_users} users tracked
          </span>
        </footer>
