...
for real-time fraud ring detection in streaming financial systems, yet its
adversarial robustness has not been systematically studied in a streaming
context.  We present \sysname, a production-grade fraud detection platform
built on Apache Flink, Memgraph, and LangGraph, and use it as an
experimental testbed for two research questions: (1)~How does \wcc-based
ring detection degrade under adversarial graph manipulation with a bounded
budget?  (2)~What is the privacy-utility tradeoff when Laplace differential
...
channels are the weakest links?  Prior adversarial graph
work~\cite{zugner2018,dai2018} operates on static graphs; we study the
\emph{streaming} case where an attacker must operate in real-time without
seeing the full accumulated graph state.
...
\begin{enumerate}
  \item A reproducible simulation harness replicating a production \wcc
        fraud ring detector (no Docker/Kafka stack required for experiments).
  \item Three adversarial attack strategies with formal budget definitions,
        evaluated across the full budget range~$B \in [0, N]$.
  \item A differential privacy velocity detector with empirical
        privacy-utility curves across $\eps \in \{0.1, \ldots, 50\}$.
  \item An empirical defense comparison: \wcc baseline
        vs.\ AdaptiveThreshold vs.\ MultiSignalDetector.
\end{enumerate}
...
problem.  Our work differs in two ways: we study \wcc rather than learned
classifiers, and we operate in a streaming setting where the adversary acts
transaction-by-transaction without access to the full accumulated graph.
...
We present the first systematic adversarial robustness study of \wcc-based
streaming fraud ring detection.
